README — Context‑Aware Drug Embedding Pipeline
Overview
...
1. Preprocess LINCS (1_preprocess_lincs)
Goal: Clean and harmonise LINCS L1000 metadata and perturbagen annotations.
Key steps
...
2. Preprocess RNA‑seq (2_preprocess_rnaseq)
Goal: Prepare expression matrices for downstream graph construction.
Key steps
...
3. Preprocess STRING (3_preprocess_string)
Goal: Build the base protein–protein interaction (PPI) graph.
Key steps
...
4. Analyse Base Graph (4_analyse_base_graph)
...
5. Generate Context Graphs (5_generate_context_graphs)
Goal: Build subgraphs representing each experimental context.
Key steps
...
6. Generate Embeddings (6_generate_embeddings)
Goal: Train a graph neural network and embed each context graph.
Key steps
...
These embeddings are analysed in notebooks 7A and 7B.

7A. Analyse ID‑Level Embeddings (7A_analyse_id_embeddings)
Goal: Visualise and interpret embeddings generated at the perturbagen ID level.
Analyses include
UMAP projection
...
7B. Analyse Name‑Level Embeddings (7B_analyse_name_embeddings)
...
- graph_base.pkl
- graph_landmark.pkl
- context_graphs/ (CID, MEAN, NAME)
- list_emb.pkl
- list_pyg.pkl
- df_umap.csv (for each analysis)
Key visualisations
- Base graph layout
- Edge‑weight histograms
- GO‑term correlation plots
- UMAP embeddings (ID and NAME)
- Neighbour‑composition barplots
- Neighbour‑enrichment scatterplots

Reproducibility & HPC Notes
The pipeline is designed for cluster environments and follows best practices:
- Centralised configuration
- Per‑sample directory isolation
- Robust error handling
- Resume‑safe processing
- Efficient job submission for graph generation and embedding training
All notebooks can be executed independently, but the recommended order is:
1 → 2 → 3 → 4 → 5 → 6 → 7A/7B
...
- a Makefile or Snakemake workflow
- a conda environment file
- a shorter README for public release
- a developer‑focused README with HPC notes
